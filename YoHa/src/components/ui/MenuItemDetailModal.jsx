...
import { useCart, makeCartKey } from '@/contexts/AppContexts.jsx';
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className="relative w-full sm:max-w-lg max-h-[92vh] sm:max-h-[88vh] overflow-y-auto bg-white dark:bg-ink-900 rounded-t-[28px] sm:rounded-3xl shadow-2xl shadow-brand-500/10 animate-slide-up sm:animate-scale-in border border-ink-200/70 dark:border-ink-800 ring-gradient"
      >
        ...
        <div className="relative px-5 pb-7 pt-1 space-y-5">
          <div>
            ...
            {item.desc ? (
              <p className="mt-3 text-[13px] text-ink-500 dark:text-ink-400 leading-relaxed">{item.desc}</p>
            ) : null}
          </div>
                  ...
                  <div key={g.name}>
                    ...
                    <div className="mt-2.5 space-y-2">
                      {(g.options || []).map((opt) => {
                        ...
                        const disabled = !active && Number(g.max) > 1 && picked.length >= Number(g.max);
                        return (
                          <button
                            type="button"
                            key={opt.name}
                          ...
                          >
                            <span className="text-sm font-semibold leading-snug">{opt.name}</span>
                            <span className="flex items-center gap-2 shrink-0">
                              ...
                              <span
                                className={`w-5 h-5 rounded-full grid place-items-center text-[11px] font-black transition-colors ${
                                  active
                                    ? 'bg-gradient-to-br from-brand-500 to-pink-500 text-white shadow-sm'
                                    : 'border-2 border-ink-300 dark:border-ink-600 text-transparent'
                                }`}
                              >
                                {Number(g.max) > 1 ? '?' : '?'}
                              </span>
          ...
          <div className="pt-1 flex flex-col gap-4">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-[10px] font-black uppercase tracking-wider text-ink-400">Prix</span>
              <div className="text-right">
                <div className="font-display font-black text-2xl bg-gradient-to-r from-brand-600 via-pink-600 to-violet-600 bg-clip-text text-transparent">
                  {formatMad(unitPrice)}
                </div>
                {optionsPrice > 0 ? (
                  <div className="text-[11px] font-semibold text-ink-400">
                    {formatMad(Number(item.price || 0))}
                    {selectedOptions.length > 0 ? ` + ${formatMad(optionsPrice)} d'options` : ''}
                  </div>
                ) : null}
              </div>
            </div>
            ...
            {orderingDisabled ? (
              ...
              <div className="flex items-center justify-between bg-gradient-to-r from-brand-500/5 via-pink-500/5 to-violet-500/5 dark:from-brand-500/10 dark:via-pink-500/5 dark:to-violet-500/10 p-1.5 rounded-2xl border border-brand-500/20 dark:border-brand-500/25">
                <button
                  ...
                  className="cursor-grow w-11 h-11 rounded-xl bg-white dark:bg-ink-900 border border-ink-200 dark:border-ink-800 text-ink-700 dark:text-ink-200 font-black text-base grid place-items-center active:scale-95"
                  aria-label="Moins"
                >
                  ?
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setQty(key, quantity + 1)}
                  className="cursor-grow w-11 h-11 rounded-xl bg-gradient-to-br from-brand-500 to-pink-500 text-white font-black text-base grid place-items-center active:scale-95 shadow-glow"
                  aria-label="Plus"
                >
                  +
                </button>
