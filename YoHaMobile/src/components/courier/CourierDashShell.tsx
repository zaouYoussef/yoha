...
export function CourierDashShell({ title, subtitle, children, contentStyle }: Props) {
    ...
    <PremiumBackground variant="cream">
      <View style={[styles.root, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 96 }]}>
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            <Text style={styles.kicker}>🚴 Dashboard livreur</Text>
            <Text style={styles.title}>{title}</Text>
            <Text style={styles.sub}>
              {subtitle || (user ? `Connecté en tant que ${user.displayName}` : '')}
            </Text>
          </View>
